...
describe("Token Unlocking Suite 🔓", () => {
  ...
  // ==========================================
  // TEST CASE 3: Token-2022 Unlock with Metaplex Metadata
  // ==========================================
  it("✅ Token-2022 Unlock with Metaplex Metadata", async () => {
    console.log(
      "\n--- Starting Test: Token-2022 Unlock with Metaplex Metadata ---",
    );
    try {
      const tokenMint = setup.token2022MetaplexMint;
      assert(
        tokenMint !== null,
        "❌ setup.token2022MetaplexMint is null (Lock test might have failed)",
      );
      ...
      const tx = await setup.program.methods
        ...
        .rpc();
      console.log(
        `✅ Unlock Token-2022 with Metaplex Metadata Tx Signature: ${tx}`,
      );
      ...
      console.log(
        "✅ Token-2022 with Metaplex Metadata Unlock verification complete and correct.",
      );
    } catch (err: any) {
      console.error(
        "❌ Caught error during Token-2022 with Metaplex Metadata Unlock:",
        err,
      );
      const signature =
        err?.signature ?? err?.txSig ?? err?.transactionSignature;
